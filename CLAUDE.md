...
- **Final polish pass (human's second reference image, 2026-08-17).** Five targeted changes, no architecture touched. (1) The top-right circle is now a **rubber stamp built from the data** — `renderStamp()` draws the shelter of the record on top of the deck as arced mono text around a ruled ring with a paw, wobbled by an SVG turbulence displacement so it reads as struck rather than typeset; it re-strikes on every record change and the type auto-sizes so the longest name ("Catering to Cats & Dogs Inc") still fits inside the ring. Nothing is invented — the words are `dog.source.shelter`. (2) The **FLAT/DESIGNED switch is now an object**: both halves are always coloured (near-black ink / warm amber), the off half is veiled toward the paper rather than emptied, and a larger detent knob rides the seam and leans toward the live side. This is the page's central idea, so it now looks like hardware instead of a form control. (3) The **stack** gained hairline edges and slightly wider offsets so it reads as separate sheets, and the active card's shadow was deepened to keep it dominant. (4) **Tabs** got folder edges — inset highlight, vertical hairlines, the front of the file lifted 5px onto the paper tone with its red number and struck rule. (5) The masthead gained hairline dividers between its registers. **Verified by rendering:** 1440×900 (masthead 170px, card 1130×592, tabs ending at y=898 — whole interaction in one viewport) and 390px; flat and designed states; the stamp checked at three different shelters. 32 tests pass, jsdom interaction check reports no errors, seven hex colours and no eighth.
